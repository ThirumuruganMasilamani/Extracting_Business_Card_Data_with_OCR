...
#page congiguration
st.set_page_config(page_title= "BizCardX App",
                   page_icon= ':hamburger:',
                   layout= "wide",)
st.markdown("<h1 style='text-align: center; color: purple;'>BizCardX</h1>", unsafe_allow_html=True)
...
def app_bg():
    st.markdown(f""" <style>.stApp {{
                        background: url("https://img.freepik.com/premium-vector/white-shade-abstract-background_450825-51.jpg");
...
with navigation:
    ...
    selected = option_menu('MainMenu', ['Home',"Image to Text","Database"],
...
with text_process:
    if selected == 'Home':
        ...
        with right:
            gif_path = 'https://nanonets.com/blog/content/images/2020/09/landing-ocr-1.gif'
            st.image(gif_path)
            
            st.write('### TECHNOLOGIES USED')
            st.write('##### *:violet[Python]  *:violet[Streamlit] *:violet[EasyOCR]  *:violet[MySQL]')
            st.write(
                "To Learn more about easyOCR [Kindly press here](https://pypi.org/project/easyocr/) ")


            with left:
                st.markdown("### Hello there! Welcome to My application")
                st.markdown('###### Extracting business card data with OCR streamlines the process of digitizing contact information. OCR technology scans the text on business cards and converts it into editable, searchable data. This enables quick and efficient extraction of key details like names, phone numbers, emails, and addresses. With OCR, businesses can effortlessly import and organize business card data into digital formats for seamless integration and access.')
...
#===================text extraction Extracting image============================
                if st.button('TEXT EXTRACTING'):
        ...
        with text:
            left,right=st.tabs([' ','Text Extraction'])
            with left:
                st.write('Please Select file and Click on the above Text Extraction ')
